  ...
  return (
    ...
    >
      <div className="relative bg-[#D9D9D938]/78 hover:bg-[#D9D9D938]/90 lg:px-6 px-4 py-3 text-center min-w-[250px] lg:min-w-[200px] whitespace-nowrap w-full">
        ...
        <p className="text-[#a89a84] font-orbitron font-bold">
          {option.optionText}
        </p>
      </div>
    </motion.div>
...
const Quiz = () => {
  ...
  const authApiUrl = `${backendUrl}/auth?redirect_uri=${encodeURIComponent(
    "/quiz"
  )}`;
  ...
  useEffect(() => {
    const fetchTodayQuiz = async () => {
      ...
      try {
        ...
        if (res.status === 400) {
          setQuizError("You've Already answered Today's Quiz");
          return;
        }
  ...
  return (
    <div>
      ...
      <img
        ...
        className="absolute size-[400px] md:size-[600px] bottom-20 right-0"
      />
        ...
        <div className="w-full h-screen flex flex-col">
          <div className="w-full flex flex-row justify-between items-center px-2 py-4">
            <div className="flex flex-row justify-center items-center space-x-2">
              <a
                href="/"
                className="font-['Orbitron'] text-md font-medium border border-transparent hover:border hover:border-[#a69188]/50 px-4 py-2 transition duration-150 ease-in-out"
              >
                &larr; Home
              </a>
          ...
          <div className="w-full flex flex-row justify-center items-center px-2 py-4 mt-8">
            {loading ? (
              ...
              <div className="w-full flex flex-col items-center justify-between space-y-6 h-[80vh]">
                <h2 className="font-khinterference text-white text-3xl">
                  Quiz of the day
                </h2>
                {!user ? (
                  <div className="flex flex-col justify-center items-center space-y-16 text-2xl -mt-10">
                    <p className="font-orbitron text-xl text-center">
                      You need to sign in to participate
                    </p>
                    <a
                      className="bg-Button font-orbitron px-6 py-4"
                      href={authApiUrl}
                    >
                      {"Sign in with Google"}
                    </a>
                    <p className="font-orbitron text-lg text-white/70 text-center">
                      Use the same ID as Devfolio to avail quiz advantage during
                      the event
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col justify-center items-center space-y-12 w-full">
                    <motion.h2
                      ...
                      transition={{ duration: 1.5 }}
                      viewport={{ once: true }}
                      className="font-khinterference md:text-5xl text-2xl text-center"
                    >
                      {quizQuestion.questionTitle}
                    </motion.h2>
                    ...
                    <motion.p
                      ...
                      transition={{ duration: 1.5 }}
                      viewport={{ once: true }}
                      className="text-lg text-center font-khinterference text-white/80"
                    >
                      Click on any option to submit your answer
                    </motion.p>
                  </div>
                ) : submitted ? (
                  ...
                  <div className="flex flex-col justify-center items-center space-y-16">
                    <motion.p
                      ...
                      transition={{ duration: 1.5 }}
                      viewport={{ once: true }}
                      className="font-khinterference text-4xl text-center md:text-6xl"
                    >
                      {quizError}
                    </motion.p>
                    <motion.p
                ...
                <div className="flex flex-row py-6 space-x-4">
                  ...
                  />
                  <motion.h4 className="text-sm min-[440px]:text-xl sm:text-2xl font-khinterference text-bold tracking-wider">
                    WELCOME{user ? `, ${user.name}` : " CODER"}
                  </motion.h4>
                  <img
                    src="/svg/quiz-ui/divider.svg"
